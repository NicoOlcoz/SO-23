...
{
	//TODO: Ejercicio 2
	//1) Buscar el grupo del inodo que nos dan.
	//2) Buscar el bloque donde está el inodo dentro del grupo que encontramos antes.
	//3) Encontrar el offset del inodo dentro de la tabla para poder leerlo.
	//4) Leer el inodo.
	// Vamos a necesitar:
		//Cantidad de inodos_por_grupo
		//En donde empieza la tabla dentro de un grupo
		//Offset del inodo en la tabla

	unsigned int group_number = blockgroup_for_inode(inode_number); //Obtengo el grupo en el que está el inodo que me pasaron
	Ext2FSBlockGroupDescriptor * group_address = block_group(group_number); //Voy a leer el grupo que obtuve antes.
	unsigned int inode_table_address = group_address -> inode_table; //Obtengo la dirección de la tabla de inodos del grupo.
	unsigned int inode_table_offset = blockgroup_inode_index(inode_table_address); //Obtengo el offset del inodo dentro de la tabla.
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	//TODO: Ejercicio 1
	unsigned int res;
	unsigned int block_size = (1024 << _superblock->log_block_size)/sizeof(unsigned int);
	unsigned char* block_buffer = (unsigned char*) malloc(1024 << _superblock->log_block_size);//Para un solo bloque
	if (block_number < 12 ){// Tablas dierctas
		return inode -> block[block_number];
	} else if ( block_number < 12 + block_size ){// Indirección simple
		unsigned int block_address = inode -> block[12];
		read_block(block_address, block_buffer);
		
		unsigned int offset_indireccion_simple = block_number - 12; // Voy al índice que me piden dentro del bloque de indirección simple.
		res = ((unsigned int *) block_buffer)[offset_indireccion_simple];
		}
	else if ( 12 + block_size <= block_number ){// Indirección doble. Voy a tener un bloque de memoria, que se cuanto lugar tiene (log_block_size) y se  que lo uso para meter
			// n punteros a bloques de memoria. 

			unsigned int block_address = inode -> block[13];
			unsigned int offset_indireccion_doble = block_number - block_size - 12;//Calculo el indice dentro del primer bloque, el de indirección doble.
			unsigned int offset_indireccion_simple = offset_indireccion_doble / block_size;//Calculo en qué bloque me voy a meter.
			
			read_block(block_address, block_buffer);//Pongo en el buffer el bloque 13. Usamos este de buffer_doble
			
			unsigned char* block_buffer_simple = (unsigned char*) malloc(1024 << _superblock->log_block_size);

			unsigned int block_address_simple = ((unsigned int *) block_buffer)[offset_indireccion_simple];//Dentro del bloque 13, accedo al bloque de memoria
			read_block(block_address_simple, block_buffer_simple);
			res = ((unsigned int *) block_buffer_simple)[offset_indireccion_simple];
	}
	return res;
}
